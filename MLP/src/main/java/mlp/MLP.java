...
public class MLP {
	...
	private void setInputLayer(double[] data) {
		...
		for (int i = 0; i < d; i ++) {
			...
			Neuron inputLayerNeuron = NeuronFactory.getNeuron(i, inputNeuronInput, this.learningRate);
		...
		for (int i = 1; i < numOfNeurons.length; i ++) {
			...
			for (int j = 0; j < numOfNeurons[i]; j ++) {
				...
				if (i == numOfNeurons.length - 1) {
					...
					// output layer neuron use logistic activate function by default
					// as this NN is used for a classification problem.
					currentHiddenLayerNeuron = NeuronFactory.getNeuron(j, layers.get(i - 1).getLayerOutputs(), 
							"logistic", this.learningRate, expectedOutput);
				}
				else {
					currentHiddenLayerNeuron = NeuronFactory.getNeuron(j, layers.get(i - 1).getLayerOutputs(), 
							activationFunc, this.learningRate);
				}
	...
	private List<Integer> generateRandomIntList(int max) {
		Random random = new Random();
		
		
		List<Integer> randomList = new ArrayList<Integer>();

		while (randomList.size() < batchSize) {
			int randomInt = random.nextInt(max);
			
			if (!randomList.contains(randomInt)) randomList.add(randomInt);
		}
	...
	public void fit (List<double[]> X, List<Integer> y, double threshold) {
		...
		double initialLearningRate = this.learningRate;
		...
		while (!checkIfTerminate(oldSME, SME, threshold, epoch)) {	
			...
			// batch for SGD: choose uniformly and randomly a batch of size batchSize to update the weights
			List<Integer> batch = new ArrayList<Integer>();
			
			if (!(this.batchSize == X.size())) batch = generateRandomIntList(X.size());
			
			
			this.learningRate = initialLearningRate / epoch;  // n = n0/k.
			...
			for (int i = 0; i < X.size(); i ++) {
				...
				if (batch.contains(i) || this.batchSize == X.size()) 
					backprop();	
			}
