...
public abstract class Neuron {
	...
	private double calculatePartialDerivative(Neuron previousLayerNeuron) {
		...
		return this.delta * previousOutput;  					 	// dE/dw(h)ij = ä(h)i * y(h-1)j
	}
	...
	public void updateWeights() {
		...
		for (int i = 0; i < previousLayerNeurons.size(); i ++) {
			...
			double newWeight = this.weights.get(i) - (this.learningRate * calculatePartialDerivative(currentNeuron)); // w(t+1) = w(t) - ç*dE/dwi
